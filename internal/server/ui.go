...
async function load(){var r=await fetch(A+'/entries').then(function(r){return r.json()});items=r.entries||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.type||'').toLowerCase().includes(q)||(i.endpoint||'').toLowerCase().includes(q)||(i.version||'').toLowerCase().includes(q)});
...
if(i.endpoint)h+='<span>'+esc(i.endpoint)+'</span>';
if(i.version)h+='<span>'+esc(i.version)+'</span>';
if(i.metadata)h+='<span>'+esc(i.metadata)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/entries/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",type:"",endpoint:"",version:"",metadata:"",status:"",tags:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' REGISTRY</h2>';
...
h+='<div class="fr"><label>Endpoint</label><input id="f-endpoint" value="'+esc(i.endpoint)+'"></div>';
h+='<div class="fr"><label>Version</label><input id="f-version" value="'+esc(i.version)+'"></div>';
h+='<div class="fr"><label>Metadata</label><input id="f-metadata" value="'+esc(i.metadata)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),type:document.getElementById('f-type').value.trim(),endpoint:document.getElementById('f-endpoint').value.trim(),version:document.getElementById('f-version').value.trim(),metadata:document.getElementById('f-metadata').value.trim(),status:document.getElementById('f-status').value.trim(),tags:document.getElementById('f-tags').value.trim()};
if(editId){await fetch(A+'/entries/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/entries',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
