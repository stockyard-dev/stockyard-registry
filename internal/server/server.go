...
s.mux.HandleFunc("GET /api/tier",func(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"tier":s.limits.Tier,"upgrade_url":"https://stockyard.dev/registry/"})})
return s}
...
func(s *Server)list(w http.ResponseWriter,r *http.Request){
    ...
    if q!=""||len(filters)>0{wj(w,200,map[string]any{"entries":oe(s.db.Search(q,filters))});return}
    wj(w,200,map[string]any{"entries":oe(s.db.List())})
}
func(s *Server)create(w http.ResponseWriter,r *http.Request){if s.limits.MaxItems>0{items:=s.db.List();if len(items)>=s.limits.MaxItems{we(w,402,"Free tier limit reached. Upgrade at https://stockyard.dev/registry/");return}};var e store.Entry;json.NewDecoder(r.Body).Decode(&e);if e.Name==""{we(w,400,"name required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
func(s *Server)update(w http.ResponseWriter,r *http.Request){
    ...
    var patch store.Entry;json.NewDecoder(r.Body).Decode(&patch);patch.ID=existing.ID;patch.CreatedAt=existing.CreatedAt
    if patch.Name==""{
patch.Name=existing.Name};if patch.Type==""{
patch.Type=existing.Type};if patch.Endpoint==""{
patch.Endpoint=existing.Endpoint};if patch.Version==""{
patch.Version=existing.Version};if patch.Metadata==""{
patch.Metadata=existing.Metadata};if patch.Status==""{
...
func(s *Server)health(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"status":"ok","service":"registry","entries":s.db.Count()})}
